...
# install-gilchrist.ps1 - Automated gilchrist installation script
...
function Show-Help {
    Write-Host "gilchrist Installer Script"
    Write-Host "======================"
    Write-Host ""
    Write-Host "USAGE:"
    Write-Host "  .\install-gilchrist.ps1           # Install system-wide (requires admin)"
    Write-Host "  .\install-gilchrist.ps1 -UserOnly # Install for current user only"
    Write-Host "  .\install-gilchrist.ps1 -Help     # Show this help"
    Write-Host ""
...
Write-Host "gilchrist Installation Script"
Write-Host "========================="
Write-Host ""

# Check if gilchrist.exe exists
if (-not (Test-Path "gilchrist.exe")) {
    Write-Host "[ERROR] gilchrist.exe not found in current directory!"
    Write-Host "Please make sure gilchrist.exe is present in the same folder as this script."
    exit 1
}

Write-Host "Found gilchrist.exe - ready for installation."
Write-Host ""

# Ask about renaming
Write-Host "Do you want to rename the executable? (default: gilchrist)"
$rename = Read-Host "Rename? (y/N)"

$exeName = "gilchrist.exe"
...
try {
    ...
    Copy-Item "gilchrist.exe" $destPath -Force
    Write-Host "[OK] Copied gilchrist.exe to $destPath"
} catch {
